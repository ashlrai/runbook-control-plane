...
**Checkout reference:** MCP server `runbook` **v0.3.2**, **33** tools in `TOOL_NAMES`.
...
| 5 | **Closed MCP inventory count + version** | `SERVER_VERSION = "0.3.2"`; `TOOL_NAMES` length **33** in `packages/mcp/src/surface.ts`; discoverable via `runbook_list_surface` | Older docs that say “20 tools” / `0.2.0`, “30 tools” / `0.3.0`, or `0.3.1` are stale relative to this file |
...
Public/operator docs should match `surface.ts` (`33` tools, `0.3.2`):
