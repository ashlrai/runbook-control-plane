...
**Checkout reference:** MCP server `runbook` **v0.4.0**, **38** tools in `TOOL_NAMES`.
...
| 5 | **Closed MCP inventory count + version** | `SERVER_VERSION = "0.4.0"`; `TOOL_NAMES` length **38** in `packages/mcp/src/surface.ts`; discoverable via `runbook_list_surface` | Older docs that say “20 tools” / `0.2.0`, “30 tools” / `0.3.0`, or `0.3.1` are stale relative to this file |
...
| 5c | **Elite process tools (surface lock, tick, seal, drift)** | `runbook_surface_lock_receipt`, `runbook_process_tick`, `runbook_session_seal_capsule`, `runbook_session_import_pack`, `runbook_drift_sentinel` in `elite-tools.ts`; surface **0.4.0** / **38** tools | Surface lock attests Runbook only, not host MCP set; seal is self-asserted synthetic capsule |
...
Public/operator docs should match `surface.ts` (`38` tools, `0.4.0`):
