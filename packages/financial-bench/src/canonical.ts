...
function assertUnicode(value: string) {
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code >= 0xd800 && code <= 0xdbff) {
      ...
      if (!Number.isInteger(next) || next < 0xdc00 || next > 0xdfff) throw new Error("bench.invalid-unicode");
      index += 1;
    } else if (code >= 0xdc00 && code <= 0xdfff) {
      throw new Error("bench.invalid-unicode");
    }
  }
}
...
export function canonicalizeJcs(value: unknown): string {
  ...
  const visit = (current: unknown, depth: number): string => {
    nodes += 1;
    if (depth > 64 || nodes > 100_000) throw new Error("bench.input-too-complex");
    ...
    if (typeof current === "number") {
      if (!Number.isFinite(current)) throw new Error("bench.invalid-number");
      return JSON.stringify(current);
    }
    if (Array.isArray(current)) return `[${current.map((item) => visit(item, depth + 1)).join(",")}]`;
    if (!isRecord(current)) throw new Error("bench.invalid-value");
...
// FIPS 180-4 SHA-256. Kept local so receipts are synchronous and browser-safe.
export function sha256Bytes(input: Uint8Array): string {
  ...
  try {
    ...
    if (byteLengthGetter === undefined || tagGetter === undefined ||
      tagGetter.call(input) !== "Uint8Array") throw new Error("bench.invalid-bytes");
    const byteLength = byteLengthGetter.call(input) as unknown;
    if (!Number.isSafeInteger(byteLength)) throw new Error("bench.invalid-bytes");
    bytes = new Uint8Array(byteLength as number);
    Uint8Array.prototype.set.call(bytes, input);
  } catch {
    throw new Error("bench.invalid-bytes");
  }
  ...
  for (let offset = 0; offset < data.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const t1 = ((h ?? 0) + s1 + choose + (constants[index] ?? 0) + (words[index] ?? 0)) >>> 0;
